...
export default async function FornecedoresPage() {
  ...
  const fornecedoresComResumo =
    fornecedores.map((fornecedor) => {

      const totalRegistado =
        fornecedor.fornecimentos.reduce(
          (total, fornecimento) =>
            total + Number(fornecimento.quantidade),
          0
        )

      const totalConfirmado =
        fornecedor.fornecimentos
          .filter(
            (fornecimento) =>
              fornecimento.estado === 'CONFIRMADO'
          )
          .reduce(
            (total, fornecimento) =>
              total + Number(fornecimento.quantidade),
        ...
        fornecedor.fornecimentos
          ...
          .reduce(
            (total, fornecimento) =>
              total +
              Number(
                fornecimento.quantidadeDisponivel
              ),
            0
          )

      const ultimoFornecimento =
        fornecedor.fornecimentos[0] ?? null
  ...
  const totalFornecedores =
    fornecedoresComResumo.length

  const totalRegistadoGeral =
    fornecedoresComResumo.reduce(
      (total, fornecedor) =>
        total + fornecedor.totalRegistado,
      0
    )
  ...
  const totalConfirmadoGeral =
    fornecedoresComResumo.reduce(
      (total, fornecedor) =>
        total + fornecedor.totalConfirmado,
      0
    )
  ...
  const totalPendenteGeral =
    fornecedoresComResumo.reduce(
      (total, fornecedor) =>
        total + fornecedor.totalPendente,
      0
    )
    ...
    <main className="min-h-screen bg-[#fff8e8]">
      ...
      <div className="mx-auto w-full max-w-[1600px] px-5 py-8 sm:px-8 lg:px-10">
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div>
              ...
              <p className="mt-4 text-sm leading-6 text-[#806b55]">
                Acompanhe quem forneceu, o que forneceu,
                quantidades, lotes, disponibilidade e
        ...
        <section>
          ...
          {fornecedoresComResumo.length === 0 ? (
            ...
            <div className="rounded-3xl border border-dashed border-[#dfceb4] bg-white p-12 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#806b55]">
                ...
                com o perfil de fornecedor, ele aparecerá
                      ...
                      (fornecedor) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-5 text-right">
                            ...
                            <Link
                              href={`/gestao/fornecedores/${fornecedor.id}`}
