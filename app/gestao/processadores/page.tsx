...
function formatarData(data: Date) {
  return new Intl.DateTimeFormat('pt-MZ').format(data)
}

export default async function ProcessadoresPage() {
  ...
  const processadoresComResumo =
    processadores.map((processador) => {

      const totalRegistado =
        processador.processamentos.reduce(
          (total, processamento) =>
            total + Number(processamento.quantidade),
          0
        )

      const totalConfirmado =
        processador.processamentos
          .filter(
            (processamento) =>
              processamento.estado === 'CONFIRMADO'
          )
          .reduce(
            (total, processamento) =>
              total + Number(processamento.quantidade),
      ...
      const ultimoProcessamento =
        processador.processamentos[0] ?? null
  ...
  const totalProcessadores =
    processadoresComResumo.length

  const totalRegistadoGeral =
    processadoresComResumo.reduce(
      (total, processador) =>
        total + processador.totalRegistado,
      0
    )
  ...
  const totalConfirmadoGeral =
    processadoresComResumo.reduce(
      (total, processador) =>
        total + processador.totalConfirmado,
      0
    )
  ...
  const totalPendenteGeral =
    processadoresComResumo.reduce(
      (total, processador) =>
        total + processador.totalPendente,
      0
    )
    ...
    <main className="min-h-screen bg-[#fff8e8]">
      ...
      <div className="mx-auto w-full max-w-[1600px] px-5 py-8 sm:px-8 lg:px-10">
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div>
              ...
              <p className="mt-4 text-sm leading-6 text-[#806b55]">
                Acompanhe quem processou, o que foi
                processado, lotes, quantidades,
                disponibilidade e estado de confirmação.
              </p>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div className="rounded-2xl border border-[#e7d5b8] bg-white px-5 py-4 shadow-sm">
                ...
                <p className="mt-1 text-2xl font-black text-[#382515]">
                  {formatarNumero(totalRegistadoGeral)}
              ...
              <div className="rounded-2xl border border-[#e7d5b8] bg-white px-5 py-4 shadow-sm">
                ...
                <p className="mt-1 text-2xl font-black text-green-700">
                  {formatarNumero(totalConfirmadoGeral)}
                  <span className="ml-1 text-xs font-bold">
                    kg
                  </span>
                </p>
              ...
              <div className="rounded-2xl border border-[#e7d5b8] bg-white px-5 py-4 shadow-sm">
                ...
                <p className="mt-1 text-2xl font-black text-orange-600">
                  {formatarNumero(totalPendenteGeral)}
                  <span className="ml-1 text-xs font-bold">
                    kg
                  </span>
                </p>
        ...
        <section>
          ...
          {processadoresComResumo.length === 0 ? (
                ...
                <table className="w-full min-w-[1500px] border-collapse">
                  ...
                  <thead>
                    ...
                    <tr className="border-b border-[#eadbc4] bg-[#fffaf1]">
                      ...
                      <th className="px-5 py-4 text-right text-xs font-extrabold uppercase tracking-wide text-[#806b55]">
                        Detalhes
                      </th>
                      ...
                      (processador) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-5">
                            ...
                            {processador.ultimoProcessamento ? (
                          ...
                          <td className="px-5 py-5 text-right">
                            ...
                            <Link
                              href={`/gestao/processadores/${processador.id}`}
                              className="inline-flex items-center gap-2 rounded-xl bg-[#166534] px-4 py-2.5 text-xs font-extrabold text-white transition hover:bg-[#14532d]"
                            >
                              Ver tudo
